document.addEventListener("DOMContentLoaded", () => {
	localStorage.getItem("data-night") ? ($(".joe_header__above-mode .icon-1").addClass("active"), $(
			".joe_header__above-mode .icon-2").removeClass("active")) : ($("html").removeAttr("data-night"), $(
				".joe_header__above-mode .icon-1").removeClass("active"), $(".joe_header__above-mode .icon-2")
			.addClass("active")), $(".joe_header__above-mode").on("click", () => {
			localStorage.getItem("data-night") ? ($(".joe_header__above-mode .icon-1").removeClass(
				"active"), $(".joe_header__above-mode .icon-2").addClass("active"), $("html")
				.removeAttr("data-night"), localStorage.removeItem("data-night")) : ($(
				".joe_header__above-mode .icon-1").addClass("active"), $(
				".joe_header__above-mode .icon-2").removeClass("active"), $("html").attr(
				"data-night", "night"), localStorage.setItem("data-night", "night"))
		}), Joe.IS_MOBILE || "off" === Joe.DYNAMIC_BACKGROUND || !Joe.DYNAMIC_BACKGROUND || Joe
		.WALLPAPER_BACKGROUND_PC || $.getScript(window.Joe.THEME_URL +
			`assets/backdrop/${Joe.DYNAMIC_BACKGROUND}`), $(".joe_header__above-search .input").on("click",
		e => {
			...
			o.css("top", n), "hover" === a ? $(this).hover(() => $(this).addClass("active"), () => $(this)
				.removeClass("active")) : ($(this).on("click", function(e) {
				$(this).toggleClass("active"), $(document).one("click", () => $(this)
					.removeClass("active")), e.stopPropagation()
			}), o.on("click", e => e.stopPropagation()))
		}), $(".joe_dropdown__link").has(".joe_dropdown__submenu").hover(function() {
			$(this).children(".joe_dropdown__submenu").stop().toggleClass("active")
		}); {
		let e = null;
		const t = () => (document.documentElement.scrollTop || document.body.scrollTop) > 300 ? $(
	...
	if ($(".joe_aside__item.timelife").length) {
		let e = [{
			...
			let t = (new Date).getFullYear(),
				...
				s = o / n * 100;
			e[2].num = o, e[2].percent = parseInt(s) + "%"
		} {
	...
	if ($(".joe_aside__item.tags").length) {
		const e = [],
			t = ["#F8D800", "#0396FF", "#EA5455", "#7367F0", "#32CCBC", "#F6416C", "#28C76F", "#9F44D3",
	...
	if ($(".joe_aside__item.flatterer").length) {
		const e = [
			...
			o = () => {
				$(".joe_aside__item.flatterer .content").html(e[t(0, e.length - 1)].replace(/\*\*(.*?)\*\*/g,
					"<mark>$1</mark>")), $(".joe_aside__item.flatterer .content").attr("class",
					"content type" + t(1, 6))
			};
		o(), $(".joe_aside__item.flatterer .change").on("click", () => o())
	}
	if ("off" !== Joe.LIVE2D && Joe.LIVE2D && $.getScript(
			...
			}), $(".joe_comment").length && $(".joe_comment__respond-type .item").on("click", function() {
			$(this).addClass("active").siblings().removeClass("active"), "draw" === $(this).attr(
				"data-type") ? ($(".joe_comment__respond-form .body .draw").show().siblings().hide(), $(
			...
			penSize: 5,
			color: "303133"
		}), $(".joe_comment__respond-form .body .draw .icon-undo").on("click", () => window.sketchpad
		.undo()), $(".joe_comment__respond-form .body .draw .icon-animate").on("click", () => window
			.sketchpad.animate(10)), $(".joe_comment__respond-form .body .draw .line li").on("click",
			function() {
				window.sketchpad.penSize = $(this).attr("data-line"), $(this).addClass("active").siblings()
					.removeClass("active")
			}), $(".joe_comment__respond-form .body .draw .color li").on("click", function() {
			window.sketchpad.color = $(this).attr("data-color"), $(this).addClass("active").siblings()
				.removeClass("active")
		...
		$(".joe_comment__reply").on("click", function() {
			const t = $(this).attr("data-coid"),
				o = $("#" + $(this).attr("data-id"));
			e.find(".joe_comment__respond-form").attr("data-coid", t), o.append(e), $(
				".joe_comment__respond-type .item[data-type='text']").click(), $(
				".joe_comment__cancle").show(), window.scrollTo({
				top: o.offset().top - $(".joe_header").height() - 15,
				behavior: "smooth"
			})
		}), $(".joe_comment__cancle").on("click", function() {
			e.find(".joe_comment__respond-form").removeAttr("data-coid"), $(".joe_comment__cancle")
				.hide(), $(".joe_comment__title").after(e), $(
					".joe_comment__respond-type .item[data-type='text']").click(), window.scrollTo({
	...
	if ($(".joe_comment").length) {
		...
		$(".joe_comment__respond-form").on("submit", function(t) {
			...
			const o = $(".joe_comment__respond-form").attr("action") + "?time=" + +new Date,
				...
				s = $(".joe_comment__respond-form .head input[name='author']").val(),
				i = $(".joe_comment__respond-form input[name='_']").val(),
				r = $(".joe_comment__respond-form .head input[name='mail']").val(),
				_ = $(".joe_comment__respond-form .head input[name='url']").val();
			let c = $(".joe_comment__respond-form .body textarea[name='text']").val();
			if ("" === s.trim()) return Qmsg.info("请输入昵称！");
			if (!/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/.test(r)) return Qmsg.info("请输入正确的邮箱！");
			if ("text" === a && "" === c.trim()) return Qmsg.info("请输入评论内容！");
			if ("draw" === a) {
				const e = $("#joe_comment_draw")[0].toDataURL("image/webp", .1);
				c = "{!{" + e + "}!} "
			}
			e || (e = !0, $(".joe_comment__respond-form .foot .submit button").html("发送中..."), $.ajax({
				url: o,
				type: "POST",
				data: {
					author: s,
				...
				success(t) {
					let o = [],
						a = "";
					o = $(t).contents(), Array.from(o).forEach(e => {
						"container" === e.parentNode.className && (a = e)
					}), /Joe/.test(t) ? window.location.reload() : (Qmsg.warning(a
						.textContent.trim() || ""), e = !1, $(
						".joe_comment__respond-form .foot .submit button").html(
						"发表评论"))
				},
				error() {
					e = !1, $(".joe_comment__respond-form .foot .submit button").html(
	...
	if ($(".comment-list__item .term .content .user .author a").each((e, t) => $(t).attr("target", "_blank")),
		...
		document.addEventListener("visibilitychange", () => {
			"hidden" === document.visibilityState ? document.title = Joe.DOCUMENT_TITLE : document
				.title = e
		})
	}
	$(".joe_header__above-slideicon").on("click", function() {
		$(".joe_header__searchout").removeClass("active"), $(".joe_header__slideout").hasClass(
			"active") ? ($("body").css("overflow", ""), $(".joe_header__mask").removeClass(
				"active slideout"), $(".joe_header__slideout").removeClass("active")) : ($("body").css(
				"overflow", "hidden"), $(".joe_header__mask").addClass("active slideout"), $(
				".joe_header__slideout").addClass("active"))
	}), $(".joe_header__above-searchicon").on("click", function() {
		$(".joe_header__slideout").removeClass("active"), $(".joe_header__searchout").hasClass(
			"active") ? ($("body").css("overflow", ""), $(".joe_header__mask").removeClass(
				"active slideout"), $(".joe_header__searchout").removeClass("active")) : ($("body").css(
				"overflow", "hidden"), $(".joe_header__mask").addClass("active"), $(
				".joe_header__searchout").addClass("active"))
	}), $(".joe_header__mask").on("click", function() {
		$("body").css("overflow", ""), $(".joe_header__mask").removeClass("active slideout"), $(
			".joe_header__searchout").removeClass("active"), $(".joe_header__slideout").removeClass(
			"active")
	}), $(".joe_header__slideout-menu .current").parents(".panel-body").show().siblings(".panel").addClass(
		"in"), $(".joe_header__slideout-menu .panel").on("click", function() {
		const e = $(this).parent().parent();
		e.find(".panel").not($(this)).removeClass("in"), e.find(".panel-body").not($(this).siblings(
				".panel-body")).stop().hide("fast"), $(this).toggleClass("in").siblings(".panel-body")
			.stop().toggle("fast")
	}); {
		const e = () => {
			...
			let a = o / 864e5,
				...
				c = 60 * (r - _),
				d = Math.floor(c);
			a = String(n).padStart(2, 0), s = String(i).padStart(2, 0), r = String(_).padStart(2, 0), c =
				String(d).padStart(2, 0), $(".joe_run__day").html(a), $(".joe_run__hour").html(s), $(
					".joe_run__minute").html(r), $(".joe_run__second").html(c)
		};
		Joe.BIRTHDAY && /(\d{4})\/(\d{1,2})\/(\d{1,2}) (\d{1,2})\:(\d{1,2})\:(\d{1,2})/.test(Joe.BIRTHDAY) && (
			e(), setInterval(e, 1e3))
	}
	$(".joe_owo__contain").length && $(".joe_owo__target").length && $.ajax({
		...
		success(e) {
			...
			for (let a in e) {
				const n = e[a];
				t += `<div class="item" data-type="${a}">${a}</div>`, o +=
			...
			$(".joe_owo__contain").html(
				...
				e.stopPropagation(), $(this).addClass("active").siblings().removeClass(
				"active");
				const t = '.joe_owo__contain .box .scroll[data-type="' + $(this).attr(
					"data-type") + '"]';
		...
		let e = Joe.MOTTO;
		e || (e = "有钱终成眷属，没钱亲眼目睹"), e.includes("http") ? $.ajax({
			url: e,
			dataType: "text",
		...
		const t = t => {
			if (window.pageYOffset >= $(".joe_header").height() && t <= 0) {
				if (e) return;
				$(".joe_header").addClass("active"), $(".joe_aside .joe_aside__item:last-child").css("top",
					$(".joe_header").height() + 15), e = !0
			} else {
				if (!e) return;
				$(".joe_header").removeClass("active"), $(".joe_aside .joe_aside__item:last-child").css(
					"top", $(".joe_header").height()), e = !1
			}
		};
		...
		document.addEventListener("scroll", () => {
			let e = Date.now();
			e - a > 15 && (t(o - window.pageYOffset), o = window.pageYOffset, o >= $(".joe_header")
				.height() ? ($(".joe_header").css({
						background: "var(--new-background)",
						"box-shadow": "0 2px 10px 0 rgba(0, 0, 0, 0.1)"
					}), $(
						".joe_header__above-nav .item,.joe_header__above-nav .joe_dropdown__link a")
					.css("color", "var(--main)"), $(
						".joe_header__above-slideicon,.joe_header__above-searchicon,.joe_header__above-sign .joe_dropdown__link .icon,.joe_header__above-mode svg"
						).css("fill", "var(--main)")) : ($(".joe_header").css({
						background: "none",
						"box-shadow": "none"
					}), $(
						".joe_header__above-nav .item,.joe_header__above-nav .joe_dropdown__link a")
					.css("color", "none"), $(".joe_header__above-nav .joe_dropdown__menu a").css(
						"color", "var(--main)"), $(
						".joe_header__above-slideicon,.joe_header__above-searchicon,.joe_header__above-sign .joe_dropdown__link .icon,.joe_header__above-mode svg"
						).css("fill", "#none"))), a = e
		})
	...
	if ($("#joe_dynamic-form").length) {
		let e = !1;
		$("#joe_dynamic-form").on("submit", function(t) {
			t.preventDefault();
			const o = $("#joe_dynamic-form").attr("action") + "?time=" + +new Date,
				a = $("#joe_dynamic-form").attr("data-type"),
				n = $("#joe_dynamic-form textarea[name='text']").val(),
				s = $("#joe_dynamic-form input[name='_']").val();
			if ("text" === a && "" === n.trim()) return Qmsg.info("你还没说点什么呢！");
			e || (e = !0, $("#joe_dynamic-form .form-foot button").html("发布中..."), $.ajax({
				url: o,
				type: "POST",
				data: {
					text: n,
				...
				error() {
					e = !1, $("#joe_dynamic-form .form-foot button").html("发布"), Qmsg
						.warning("发布失败，请刷新重试！")
				}
			}))
		})
	}
	if ($(".joe_dynamic .content img:not(img.owo_image)").each(function() {
			$(this).wrap($(
				`<span style="display: block;cursor: pointer;" data-fancybox="Joe" href="${$(this).attr("src")}"></span>`
				))
		}), $(".joe_cross").length) {
		let e = !1;
		$(".joe_cross__respond-form").on("submit", function(t) {
			t.preventDefault();
			const o = $(".joe_cross__respond-form").attr("action") + "?time=" + +new Date,
				a = $(".joe_cross__respond-form").attr("data-type"),
				n = $(".joe_cross__respond-form").attr("data-coid"),
				s = $(".joe_cross__respond-form .head input[name='author']").val(),
				i = $(".joe_cross__respond-form input[name='_']").val(),
				r = $(".joe_cross__respond-form .head input[name='mail']").val(),
				_ = $(".joe_cross__respond-form .head input[name='url']").val();
			let c = $(".joe_cross__respond-form .body textarea[name='text']").val();
			return "text" === a && "" === c.trim() ? Qmsg.info("别急，说一句呗~") : $("#author").length &&
				"" === s.trim() ? $("#author").focus() : $("#mail").length && !
				/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/.test(r) ? $("#mail").focus() : void(e || (
					e = !0, $(".joe_cross__respond-form .foot .submit button").html("发表中..."), $
					.ajax({
						url: o,
						type: "POST",
						data: {
						...
						success(t) {
							...
							o = $(t).contents(), Array.from(o).forEach(e => {
								...
								.textContent.trim() || ""), e = !1, $(
								".joe_cross__respond-form .foot .submit button").html(
								"确定"))
						},
						error() {
							e = !1, $(".joe_cross__respond-form .foot .submit button").html(
								"确定"), Qmsg.error("失败了，刷新重试~")
						}
					})))
		})
	}
	if ($(".joe_cross .content img:not(img.owo_image)").each(function() {
			$(this).wrap($(
				`<span style="display: block;cursor: pointer;" data-fancybox="Joe" href="${$(this).attr("src")}"></span>`
				))
		}), $(".joe_cross__respond").length) {
		const e = $(".joe_cross__respond");
		$(".joe_cross__reply").on("click", function() {
			const t = $(this).attr("data-coid"),
				o = $("#" + $(this).attr("data-id"));
			e.show(), e.find(".joe_cross__respond-form").attr("data-coid", t), o.append(e), $(
					".joe_cross__respond-type .item[data-type='text']").click(), $("#textarea").focus(),
				$(".joe_cross__cancle").show(), window.scrollTo({
					top: o.offset().top - $(".joe_header").height() - 15,
					behavior: "smooth"
				})
		}), $(".joe_cross__cancle").on("click", function() {
			$(".joe_cross__respond-form .head").length && e.hide(), e.find(".joe_cross__respond-form")
				.removeAttr("data-coid"), $(".joe_cross__cancle").hide(), $(".joe_cross__title").after(
					e), $(".joe_cross__respond-type .item[data-type='text']").click(), window.scrollTo({
	...
	$(".comment-list__item .term .content .user .author a").each((e, t) => $(t).attr("target", "_blank")), $(
		".joe_cross .joe_pagination a").each((e, t) => {
		const o = $(t).attr("href");
		o && o.includes("#") && $(t).attr("href", o.replace("#comments", "?scroll=joe_cross"))
		...
		$.ajax({
			...
			dataType: "json",
			success: e => {
				e.success ? ($(this).removeClass("zm zm-icon_dianzan_x").addClass(
					"zm zm-icon_dianzan_m"), $(this).text(e.count), Qmsg.success(
					"谢谢你支持~")) : Qmsg.info("明天再支持~")
			}
		})
	}), $(".joe_header__above-sign button[type='button']").on("click", function(e) {
		return "" === $(".joe_header__above-sign input[name='name']").val().trim() ? ($(
				".joe_header__above-sign input[name='name']").focus(), Qmsg.warning("请输入昵称")) : "" ===
			$(".joe_header__above-sign input[name='password']").val().trim() ? ($(
				".joe_header__above-sign input[name='password']").focus(), Qmsg.warning("请输入密码")) : ($(
					this).html("登录中...").attr("disabled", !0), void $(".joe_header__above-sign form")
				.submit())
	}), $(".joe_header__above-sign form").keydown(function() {
		var e = window.event;
		if (e && 13 == e.keyCode) {
			if ("" === $(".joe_header__above-sign input[name='name']").val().trim()) return $(
				".joe_header__above-sign input[name='name']").focus(), Qmsg.warning("请输入昵称");
			if ("" === $(".joe_header__above-sign input[name='password']").val().trim()) return $(
				".joe_header__above-sign input[name='password']").focus(), Qmsg.warning("请输入密码");
			$(this).submit()
		}
	}), $("#loginForm button[type='button']").on("click", function(e) {
		return "" === $("#loginForm input[name='name']").val().trim() ? Qmsg.warning("请输入昵称") : "" ===
			$("#loginForm input[name='password']").val().trim() ? Qmsg.warning("请输入密码") : ($(this).html(
				"登录中...").attr("disabled", !0), void $("#loginForm").submit())
	})
});
