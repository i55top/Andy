<article class="joe_detail__article">
	<?php if ($this->fields->mode === "multiple") : ?>
            <div class="joe_detail__image">
                <ul class="joe_detail__image-ranking">                        
                <?php $image = _getThumbnails($this) ?>
                <?php for ($x = 0;$x<count($image);$x++) : ?>
                <li class="item">
                <img width="100%" height="100%" src="<?php echo $image[$x]; ?>" />
                </li>
                <?php endfor; ?>
                </ul>
             </div>                       
